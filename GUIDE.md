...
The current `subsrv` implementation exposes three server configs:
...
`reality_dest`, `reality_server_names`, and `reality_short_id` are derived from `deploy/xray.py` by default.
...
- `templates/hstd/hostapd.conf.j2`: SSID, channel plan, and `country_code=RU`
...
`subsrv` listens on `:8080` with HTTPS. It is not behind nginx. It serves:
...
The JSON configs returned by `subsrv` also include built-in routing rules. This split routing is separate from the Linux-level split routing on `hstd`.
...
The routing rules in those client configs currently do this:
...
So even clients using the subscription endpoint receive split-routing behavior, not a pure full-tunnel profile.
...
Edit `templates/hstd/hostapd.conf.j2`. The SSID, channel settings, and country code are not inventory variables right now.
