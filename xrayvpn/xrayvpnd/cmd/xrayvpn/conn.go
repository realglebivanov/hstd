	...
	cmd.AddCommand(
		&cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				if err := xray_conns.Add(db, args[0]); err != nil {
					return err
				}
		...
		&cobra.Command{
			...
			Args:               cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				activeChanged, err := db.RemoveConn(args[0])
				if err != nil {
					return err
				}
		...
		&cobra.Command{
			...
			Args:               cobra.ExactArgs(1),
			RunE: func(cmd *cobra.Command, args []string) error {
				if err := db.ChooseConn(args[0]); err != nil {
					return err
				}
		...
		&cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				for _, c := range connList {
					...
					table.Append(active, c.ID, c.Summary(), c.SubID)
				}
				table.Render()
				return nil
			},
